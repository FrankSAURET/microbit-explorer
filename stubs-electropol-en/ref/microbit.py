...
MICROBIT_APIS = [
    ...
    # Pins V2
    _("microbit.pin_logo.is_touched() \nIf pin_logo is_touched() on micro:bit, return True. If nothing is touching the\npin, return False."),
    _("microbit.pin_speaker.write_analog(value) \nSet pin_speaker to output a value between 0 and 1023."),
    # I2C
    ...
    # NeoPixel module V2
    _("neopixel.NeoPixel(pin, n, bpp) \nCreate a list representing a strip of 'n' neopixels controlled from the\nspecified pin (e.g. microbit.pin0) and with bpp indicating the number of bytes per pixel (bpp). For RGBW, this is is 4 rather than the default of 3 for RGB and GRB.\nUse the resulting object to change each pixel by position (starting from 0).\nIndividual pixels are given RGB (red, green, blue) or RGBW (red, green, blue, white) values between 0-255 as a\ntupe. For example, (255, 255, 255) is white:\n\nnp = neopixel.NeoPixel(microbit.pin0, 8, 4)\nnp[0] = (255,  0, 128, 0)\nnp.show()\nIn RGBW, (255,255,255,0) or (0,0,0,255) is also white.\n\n"),
    _("neopixel.NeoPixel.write() \nShow the pixels. Must be called for any updates to become visible. Is available and is equivalent to."),
    _("neopixel.NeoPixel.fill() \nColour all pixels a given RGB/RGBW value. The colour argument should be a tuple of the same length as the mumber of bytes per pixel (bpp). For example fill((0,0,255)). Use in conjunction with show() to update the Neopixels.."),
    # Radio
    ...
    # Speech V2
    _("speech.pronounce(phonemes, pitch=64, speed=72, mouth=128, throat=128, pin=pin0) \nPronounce the phonemes in the string 'phonemes'. Override the optional pitch,\nspeed, mouth and throat settings to change the tone of voice.\nFor micro:bit V2 an optional argument to specify the output pin can be used to override the default of pin0. If we do not want any sound to play out of the pins can use pin=None."),
    _("speech.say(words, pitch=64, speed=72, mouth=128, throat=128, pin=pin0) \nSay the English words in the string 'words'. Override the optional pitch,\nspeed, mouth and throat settings to change the tone of voice.\nFor micro:bit V2 an optional argument to specify the output pin can be used to override the default of pin0. If we do not want any sound to play out of the pins can use pin=None."),
    _("speech.sing(song, pitch=64, speed=72, mouth=128, throat=128, pin=pin0) \nSing the phonemes in the string 'song'. Add pitch information to a phoneme\nwith a hash followed by a number between 1-255 like this: '#112DOWWWWWWWW'.\nOverride the optional pitch, speed, mouth and throat settings to change the\ntone of voice.\nFor micro:bit V2 an optional argument to specify the output pin can be used to override the default of pin0. If we do not want any sound to play out of the pins can use pin=None."),
    # Microphone V2
    _("microbit.microphone.current_event() \nReturns the name of the last recorded sound event, SoundEvent('loud') or SoundEvent('quiet')."),
    _("microbit.microphone.was_event(event) \nEvent is a sound event such as SoundEvent.LOUD or SoundEvent.QUIET.\nReturns true if sound was heard at least once since the last call, otherwise false. The function also clears the sound event history before returning."),
    _("microbit.microphone.is_event(event) \nEvent is a sound event such as SoundEvent.LOUD or SoundEvent.QUIET.\nReturns true if sound event is the most recent since the last call, otherwise false. The function does not clear the sound event history."),
    _("microbit.microphone.get_events() \nReturns a tuple of the event history. The most recent is listed last. The function also clears the sound event history before returning."),
    _("microbit.microphone.set_threshold(event, value) \nAn event is a sound event, such as SoundEvent.LOUD or SoundEvent.QUIET.\nValue is the threshold level in the range 0-255. For example, set_threshold(SoundEvent.LOUD, 250) will only trigger if the sound is very loud (>= 250)."),
    _("microbit.microphone.sound_level() \nReturns a representation of the sound pressure level in the range 0 to 255."),
    # Speaker V2
    _("microbit.speaker.off() \nUse off() to turn off the speaker. This does not disable sound output to an edge connector pin."),
    _("microbit.speaker.on() \nUse on() to turn on the speaker."),
]
